...
# Write the output to a new CSV file for QGIS
with open("output_for_qgis.csv", "w", encoding='utf-8', newline='') as output_file:
    writer = csv.writer(output_file)
    writer.writerow(["Pseudocode", "Similarity", "District Name"])  # Write the header row
    ...
    for pseudocode, similarity in similarities.items():
        if similarity > threshold:
            with open("129_prof1.csv", "r", encoding='utf-8') as file:
                ...
                for row in content:
                    if pseudocode == row[0]:
                        writer.writerow([pseudocode, similarity, row[2]])  # Write the relevant information to the new CSV file
                        break  # Once the matching row is found, break out of the loop
